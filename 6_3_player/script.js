/*
 Модифицировать плеер (ссылка на плеер в соответствующем канале слака) таким образом,
 чтобы с прогресс-баром можно было взаимодействовать.
 При клике - «перематывать» композицию на указанную позицию
 */
...
//Добавленный код -----------------------------------------------------------
        ...
        if (e.target.classList.contains('progress')) {
            progressElement = e.target;
        }

        if (e.target.classList.contains('progress-bar')) {
            progressElement = e.target.parentElement;
        }

        if (progressElement && e.target.closest('li') === playingItem) {
            progressPercent = e.offsetX * 100 / progressElement.getBoundingClientRect().width;
            globalPlayer.currentTime = Math.floor(globalPlayer.duration * progressPercent / 100);
        }
...
//Добавленный код -----------------------------------------------------------
                let progressBar = currentItem.children[1].firstElementChild.firstElementChild.firstElementChild;
                let progressPercent = parseInt(progressBar.style.width, 10);

                let currentTime = Math.floor(currentItem.getAttribute('data-duration') * progressPercent / 100);
